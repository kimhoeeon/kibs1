 ...
 * @param categoryValue
 * */
function kibstvYearList(pageNum, categoryValue) {
    // 데이터 조회
...
function searchPosts(pageNum, categoryValue) {
    ...
    $.ajax({
        url: '/online/kibstv_year/selectList.do',
        method: 'post',
    ...
    .done(function (data, status){
        ...
        $.each(results , function(i){
            ...
            let youtubeUrl = results[i].youtubeUrl; // https://youtu.be/wpqM_Ahpx_E?si=
            let youtubeGbn = 'long';
...
function f_kibstv_year_search(target, category){
    $(target).parent().siblings().removeClass('active');
    $(target).parent().addClass('active');

    /* 조회 */
    kibstvYearList(1, category);
}