 ...
 * */
function newsletterList(pageNum) {
    // 데이터 조회
    searchPosts(pageNum);
...
function searchPosts(pageNum) {
    ...
    $.ajax({
        url: '/board/newsletter/selectList.do',
        method: 'post',
    ...
    .done(function (data, status){
        ...
        $.each(results , function(i){
            ...
            if(noticeGbn === '1'){
                ...
                str += '<div class="tit" onClick="location.href=\'/board/newsletterView.do?id=' + id + '\'">';
                    str += title;
                str += '</div>';
                str += '<div class="write">';
                    str += writer;
        ...
        if(nvl(results,"") !== "") {
            ...
            emptyStr += '<li>';
            emptyStr += '<div>';
            emptyStr += '해당 조건으로 검색된 뉴스레터가 없습니다.';
            emptyStr += '</div>';
            emptyStr += '</li>';
