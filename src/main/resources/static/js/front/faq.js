...
function searchPosts(pageNum) {
    ...
    $.ajax({
        url: '/exhibitor/faq/selectList.do',
        method: 'post',
    ...
    .done(function (data, status){
        ...
        if(nvl(results,"") !== "") {
            ...
            emptyStr += '<div>';
            if(lang === 'KO'){
                emptyStr += '해당 조건으로 검색된 FAQ가 없습니다.';
            }else{
                emptyStr += 'No QnAs were retrieved for this condition.';
            }
            emptyStr += '</div>';
            emptyStr += '</li>';
            $('ul.box').html(emptyStr);
        }
    })
