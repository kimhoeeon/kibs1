...
$(function(){
    //페이지 오픈 시 default ()
    contestList(pageNum, '전체');

    $(document).on('click', '.cate', function(e) {
        gbn = $(e.target).data('value');
    });
});
 ...
 * @param gbnValue
 * */
function contestList(pageNum, gbnValue) {
    ...
    $(document).on('click', '.paging>ol>li>a', function() {
        if (!$(this).hasClass('this')) {
            ...
            searchPosts(Number($(this).text()), gbn);
        }
    });
    ...
    $(document).on('click', '.paging>span', function() {
        ...
        if (id === 'first_page') { //<<
            ...
            searchPosts(nextPage, gbn);
        } else if (id === 'last_page') { //>>
            searchPosts(totalPage, gbn);
        }
    });
 ...
 * @param gbnValue - Category Value
 */
function searchPosts(pageNum, gbnValue) {
    ...
    $.ajax({
        url: '/board/contest/selectList.do',
        method: 'post',
...
function f_contest_category_search(target, gbnValue){
    $(target).siblings().removeClass('active');
    $(target).addClass('active');

    /* 조회 */
    contestList(1, gbnValue);
}