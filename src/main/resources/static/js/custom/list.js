        ...
        $('#progress_step_btn').on('click', function () {
            ...
            if(checkbox_len !== 0){
                ...
                if(nvl(checkbox_val,'') !== ''){
                    ...
                    let targetList_el = $('#progressStepModalForm .target_list');
                    targetList_el.html(checkbox_data_val);
                    targetList_el.append(input_hidden);
                    targetList_el.append(input_hidden2);
...
function f_request_list_search(){
    ...
    let resData = ajaxConnect('/mng/request/management/list/selectList.do', 'post', jsonObj);
...
function f_request_list_detail_set(seq){
    window.location.href = '/mng/request/management/list/detail.do?seq=' + seq;
}
    ...
    if(nvl(seq,'') !== ''){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                let resData = ajaxConnect('/mng/request/management/list/delete.do', 'post', { seq: seq });
                        ...
                        if(md_progress_step_val === '완료'){
                            ...
                            if (nvl(titleArr,'') !== ''){
                                ...
                                for(let i=0; i<titleList.length; i++){
                                    ...
                                    let subject = '[ KIBS ] - 요청사항 & 문의 ( ' + title + ' ) 요청이 처리 완료되었습니다.';
                                    let body = '';
                                    body += '<p><br/></p>';
                                    body += '<p>';
                                    body += '--------------------------------------------------------------------------------------';
    ...
    if(validCheck){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                /* Modify */
                let processGbn = 'I';
                let url = '/mng/request/management/list/insert.do';
                if(nvl(seq, '') !== '') {
                    processGbn = 'U';
                    url = '/mng/request/management/list/update.do';
                }
                            ...
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    if(processGbn === 'U'){
                                        ...
                                        // [ KIBS ] - (오류) 요청사항 문의드립니다.
                                        let subject = '[ KIBS ] - 요청사항 & 문의 (' + form.gbn + ') 관련 새 글이 등록되었습니다.';
                                        let body = '';
                                        body += '<p>';
                                            body += 'KIBS 관리자 페이지에 요청사항 & 문의가 등록되었습니다.';
                                        body += '</p>';
                                        body += '<p><br/></p>';
                                        body += '<p><br/></p>';
                                        body += '<p>';
        ...
        if (nvl(content, '') === '') {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    /* Modify */
                    let url = '/mng/request/management/list/reply/insert.do';
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.resultCode === "0") {
                                ...
                                if (nvl($('#gbn').val(), '') === '뉴스레터'
                                    && form.note === '개발사') {
                                    f_request_list_reply_sms_send2(form);
                                }
...
function f_request_list_reply_sms_send(form){
    ...
    let subject = '[ KIBS ] - 요청사항 & 문의 (' + gbn + ') ' + title +  ' 에 새 댓글이 등록되었습니다.';
    let body = '';
    body += '<p>';
...
function f_request_list_reply_sms_send2(form){
    let gbn = $('#gbn').val();
    let title = $('#title').val();
    let content = $('#replyContent').val();
    let note = form.note;
    ...
    let cpEmail = 'eh32415@kintex.com';
    let receiverArr = [];
    receiverArr.push({
        email: cpEmail //받는이 메일주소,
    });
    ...
    if(nvl(seq,'') !== ''){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                let resData = ajaxConnect('/mng/request/management/list/reply/delete.do', 'post', { seq: seq });
