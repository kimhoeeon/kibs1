...
$(function(){
    ...
    $('input[type=radio][name=partnerYn]').on('change', function() {
...
function f_transfer_visitor_search(){
    ...
    if(nvl(searchText,'') === ''){
        ...
        jsonObj = {
            joinYear: $('#transferYear option:selected').val(),
            condition: $('#search_box option:selected').val(),
            searchText: searchText
        }
    }

    let resData = ajaxConnect('/mng/exhibitor/transfer/visitor/selectList.do', 'post', jsonObj);
    ...
    let jb = $('#kt_exhibitor_transfer_visitor_table tbody td');
    let cnt = 0;
...
function f_transfer_search_condition_box_change(){
    ...
    let joinYear = $('#transferYear option:selected').val();
...
function f_transfer_visitor_detail(seq){
    window.location.href='/mng/exhibitor/transfer/visitor/detail.do?seq=' + seq;
}

function f_transfer_visitor_save(seq){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                if(validCheck){
                    ...
                    $.ajax({
                        url: '/mng/exhibitor/transfer/visitor/update.do',
                        method: 'POST',
                        async: false,
                        data: data,
                        dataType: 'json',
                        ...
                        success: function (data) {
                            if (data.resultCode === "0") {
                                ...
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        f_transfer_visitor_detail(tableSeq); // 재조회
                                    }
                                });
...
function f_transfer_visitor_excel_export(){
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            let fileName = 'transfer_visitor_list_' + getCurrentDate() + '.xlsx';

            $.ajax({
                url: '/mng/visitor/join/download.do?fileName=' + fileName + '&joinYear=' + transferYear,
                method: 'get',
                /*async: false,*/
                xhrFields: {
                    responseType: 'blob'
