...
function f_board_newsletter_ko_search(){
    ...
    let jb = $('#kt_center_board_newsletter_ko_table tbody td');
    let cnt = 0;
...
function f_newsletter_ko_detail_modal_set(rowId){
    /* TM 및 잠재DB 목록 상세 조회 */
    let jsonObj = {
        lang: "KO",
        id: rowId
...
function f_board_newsletter_ko_remove(rowId){
    //console.log('삭제버튼');
    if(nullToEmpty(rowId) !== ""){
        ...
        Swal.fire({
            icon: 'warning',
            title: '[ 뉴스레터 ]',
            html: '<span style="font-size: 1.2em;">선택한 뉴스레터를 삭제하시겠습니까?</span>',
            ...
            if (result.isConfirmed) {
                ...
                if (resData.resultCode === "0") {
                    showMessage('', 'info', '[ 뉴스레터 ]', '뉴스레터가 삭제되었습니다.', '');
                    f_board_newsletter_ko_search(); // 삭제 성공 후 재조회 수행
                } else {
                    showMessage('', 'error', '에러 발생', '뉴스레터 삭제를 실패하였습니다. 관리자에게 문의해 주세요. ' + resData.resultMessage, '');
                }
            }
        });
    }
...
function f_board_newsletter_ko_modify_init_set(id){
    window.location.href = '/mng/center/board/newsletter/detail.do?lang=KO'+'&seq=' + id;
}

function f_board_newsletter_ko_temp_save(id){
    Swal.fire({
        icon: 'info',
        title: '[ 뉴스레터 ]',
        html: '<span style="font-size: 1.2em;">입력된 정보를 임시저장하시겠습니까?<br>( 해당 기능은 페이지 미노출 상태로 저장됩니다. )</span>',
        allowOutsideClick: false,
        showCancelButton: true,
                    ...
                    success: function (data) {
                        if (data.resultCode === "0") {
                            Swal.fire({
                                icon: 'info',
                                title: '[ 뉴스레터 ]',
                                html: '<span style="font-size: 1.2em;">뉴스레터 정보가 변경되었습니다. (임시저장)</span>',
                                allowOutsideClick: false,
                                confirmButtonColor: '#00a8ff',
                                confirmButtonText: '확인'
                            }).then((result) => {
                                if (result.isConfirmed) {
                                    f_board_newsletter_ko_modify_init_set(id); // 재조회
                                }
                            });
                        } else {
                            showMessage('', 'error', '에러 발생', '뉴스레터 정보 변경을 실패하였습니다. 관리자에게 문의해 주세요. ' + data.resultMessage, '');
                        }
                    },
                    ...
                    success: function (data) {
                        if (data.resultCode === "0") {
                            Swal.fire({
                                icon: 'info',
                                title: '[ 뉴스레터 ]',
                                html: '<span style="font-size: 1.2em;">뉴스레터 정보가 등록되었습니다. (임시저장)</span>',
                                allowOutsideClick: false,
                                confirmButtonColor: '#00a8ff',
                                confirmButtonText: '확인'
                            }).then((result) => {
                            ...
                            showMessage('', 'error', '에러 발생', '뉴스레터 정보 등록을 실패하였습니다. 관리자에게 문의해 주세요. ' + data.resultMessage, '');
                        }
                    },
                    error: function (xhr, status) {
                        alert('오류가 발생했습니다. 관리자에게 문의해 주세요.\n오류명 : ' + xhr + "\n상태 : " + status);
...
function f_board_newsletter_ko_save(id){
    //console.log(id + '변경내용저장 클릭');
    Swal.fire({
        icon: 'info',
        title: '[ 뉴스레터 ]',
        html: '<span style="font-size: 1.2em;">입력된 정보를 저장하시겠습니까?<br>( 해당 기능은 페이지 노출 상태로 저장됩니다. )</span>',
        allowOutsideClick: false,
        showCancelButton: true,
        confirmButtonColor: '#00a8ff',
        ...
        if (result.isConfirmed) {
            ...
            /* form valid check */
            let validCheck = f_board_newsletter_ko_valid();
                        ...
                        success: function (data) {
                            if (data.resultCode === "0") {
                                Swal.fire({
                                    icon: 'info',
                                    title: '[ 뉴스레터 ]',
                                    html: '<span style="font-size: 1.2em;">뉴스레터 정보가 변경되었습니다.</span>',
                                    allowOutsideClick: false,
                                    confirmButtonColor: '#00a8ff',
                                    confirmButtonText: '확인'
                                }).then((result) => {
                        ...
                        success: function (data) {
                            if (data.resultCode === "0") {
                                Swal.fire({
                                    icon: 'info',
                                    title: '[ 뉴스레터 ]',
                                    html: '<span style="font-size: 1.2em;">뉴스레터 정보가 등록되었습니다.</span>',
                                    allowOutsideClick: false,
                                    confirmButtonColor: '#00a8ff',
                                    confirmButtonText: '확인'
                                }).then((result) => {
...
function f_board_newsletter_ko_valid(){
    let title = $('#title').val();
