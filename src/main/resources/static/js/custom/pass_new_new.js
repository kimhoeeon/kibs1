        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                if(nvl(seq,"") !== ""){
                    ...
                    let resData = ajaxConnect('/mng/exhibitorNewNew/application/pass/delete.do','post',jsonObj);
...
function f_application_pass_new_search_condition_init(){
...
function f_application_pass_new_search(){
    ...
    let resData = ajaxConnect('/mng/exhibitorNewNew/application/pass/selectList.do', 'post', jsonObj);
    ...
    let jb = $('#kt_exhibitor_application_pass_new_table tbody td');
    let cnt = 0;
...
function f_application_pass_new_modify_init_set(id){
    window.location.href = '/mng/exhibitorNewNew/application/pass/detail.do?seq=' + id;
}

function f_application_pass_new_save(seq){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if(validCheck){
                ...
                $.ajax({
                    url: '/mng/exhibitorNewNew/application/pass/update.do',
                    method: 'POST',
                    async: false,
                    data: data,
                    dataType: 'json',
                    ...
                    success: function (data) {
                        if (data.resultCode === "0") {
                            ...
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    f_application_pass_new_modify_init_set(seq); // 재조회
                                }
                            });

                        } else {
                            showMessage('', 'error', '에러 발생', '출입증 신청 정보 저장을 실패하였습니다. 관리자에게 문의해 주세요. ' + data.resultMessage, '');
                        }
                    },
