...
$(function(){
    ...
    $('.addDel').on('click', function () {
        Swal.fire({
            icon: 'warning',
            title: '[ 바이어 정보 ]',
            html: '<span style="font-size: 1.2em;">해당 바이어 정보를 삭제하시겠습니까?</span>',
            ...
            if (result.isConfirmed) {
                ...
                if(nvl(seq,"") !== ""){
                    ...
                    let resData = ajaxConnect('/mng/exhibitorNewNew/application/buyer/delete.do','post',jsonObj);
...
function f_application_buyer_new_search_condition_init(){
...
function f_application_buyer_new_search(){
    ...
    let resData = ajaxConnect('/mng/exhibitorNewNew/application/buyer/selectList.do', 'post', jsonObj);
...
function f_application_buyer_new_modify_init_set(id){
    window.location.href = '/mng/exhibitorNewNew/application/buyer/detail.do?seq=' + id;
}

function f_application_buyer_new_save(seq){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if(validCheck){
                ...
                $.ajax({
                    url: '/mng/exhibitorNewNew/application/buyer/update.do',
                    method: 'POST',
                    async: false,
                    data: data,
                    dataType: 'json',
                    ...
                    success: function (data) {
                        if (data.resultCode === "0") {
                            ...
                            Swal.fire({
                                icon: 'info',
                                title: '[ 바이어 정보 ]',
                                html: '<span style="font-size: 1.2em;">바이어 정보가 저장되었습니다.</span>',
                                allowOutsideClick: false,
                                confirmButtonColor: '#00a8ff',
                                confirmButtonText: '확인'
                            }).then((result) => {
                                if (result.isConfirmed) {
                                    f_application_buyer_new_modify_init_set(seq); // 재조회
                                }
                            });
