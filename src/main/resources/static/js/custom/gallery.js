...
function f_board_gallery_search(){
    ...
    let resData = ajaxConnect('/mng/center/board/gallery/selectList.do', 'post', jsonObj);
    ...
    let jb = $('#kt_center_board_gallery_table tbody td');
    let cnt = 0;
...
function f_gallery_detail_modal_set(rowId){
    /* 목록 상세 조회 */
    let jsonObj = {
        "id": rowId
    };

    let resData = ajaxConnect('/mng/center/board/gallery/selectSingle.do', 'post', jsonObj);
...
function f_board_gallery_remove(rowId){
    //console.log('삭제버튼');
    if(nullToEmpty(rowId) !== ""){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                let resData = ajaxConnect('/mng/center/board/gallery/delete.do', 'post', jsonObj);

                if (resData.resultCode === "0") {
                    showMessage('', 'info', '자료 삭제', '자료가 삭제되었습니다.', '');
                    f_board_gallery_search(); // 삭제 성공 후 재조회 수행
                } else {
                    showMessage('', 'error', '에러 발생', '자료 삭제를 실패하였습니다. 관리자에게 문의해주세요. ' + resData.resultMessage, '');
                }
            }
        });
    }
...
function f_board_gallery_modify_init_set(id){
    window.location.href = '/mng/center/board/gallery/detail.do?seq=' + id;
}

function f_board_gallery_save(id){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if(validCheck){
                ...
                if(fileIdList !== ''){
                    let boardNoticeForm = document.getElementById('galleryForm');
                ...
                let serialData = JSON.parse(JSON.stringify($('#galleryForm').serializeArray()));
                ...
                if(nvl(id, "") !== ""){
                    $.ajax({
                        url: '/mng/center/board/gallery/modifySave.do',
                        method: 'POST',
                        async: false,
                        ...
                        success: function (data) {
                            if (data.resultCode === "0") {
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        f_board_gallery_modify_init_set(id); // 재조회
                                    }
                                });
                            } else {
                                showMessage('', 'error', '에러 발생', '자료 정보 변경을 실패하였습니다. 관리자에게 문의해주세요. ' + data.resultMessage, '');
                            }
                        },
                    ...
                    $.ajax({
                        url: '/mng/center/board/gallery/insertSave.do',
                        method: 'POST',
                        async: false,
                        data: JSON.stringify(data),
                        dataType: 'json',
                        ...
                        success: function (data) {
                            if (data.resultCode === "0") {
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        window.location.href = '/mng/center/board/gallery.do'; // 목록으로 이동
                                    }
                                });
                            } else {
                                showMessage('', 'error', '에러 발생', '자료 정보 등록을 실패하였습니다. 관리자에게 문의해주세요. ' + data.resultMessage, '');
                            }
                        },
                        error: function (xhr, status) {
                            alert('오류가 발생했습니다. 관리자에게 문의해주세요.\n오류명 : ' + xhr + "\n상태 : " + status);
